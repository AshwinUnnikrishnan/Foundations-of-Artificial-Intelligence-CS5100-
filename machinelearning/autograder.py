...
@test('q4', points=7)
def check_dqn(tracker):
    # Check compute_q_target
    from backend import Transition

    test_layout = layout.getLayout('smallGrid')
    nographics = False
    pacmanType = loadAgent("PacmanDeepQAgent", nographics)
    pacman = pacmanType(test_layout)

    pacman.discount = 0.9
    pacman.model = DummyDeepQModel()
    pacman.target_model = DummyTargetModel()
    pacman.td_error_clipping = None
    pacman.counts = np.ones((7, 7))
    pacman.counts[(5, 5)] = 4
    pacman.counts[(4, 2)] = 8
    pacman.counts[(3, 1)] = 7
    ...
    actions = [4, 1, 1, 4, 3, 0, 2, 1, 2, 2]
    rewards = [4, 4, 1, 3, 2, 0, 4, 0, 0, 4]
    dones = [0] * 10
    dones[7] = 1
    
    minibatch = [Transition(s, a, r, sp, d) for s, a, r, sp, d in zip(states, actions, rewards, next_states, dones)]

    targets = pacman.compute_q_targets(minibatch)
    expected_targets = np.array([
        [-1.04959974,  0.28053207, -2.4450277 ,  0.74411058,  8.06777848],
        [ 1.6450382 ,  6.94955885,  0.32449569,  0.67251539, -0.15441748],
        [ 1.06687712,  5.80459777,  1.58102358,  0.12312108, -1.4874284 ],
        [ 0.97399501,  0.77556107, -1.36586018, -0.31102273,  6.89425669],
        [ 1.43362316, -0.1287049 ,  0.49881204,  6.9087209 , -0.41563865],
        [ 2.00084122,  0.50403613, -1.19492753, -0.18776298, -0.88418301],
        [-0.47202491,  0.14364718,  8.52487796, -0.18619758, -0.62891215],
        [-1.97911112,  5.        , -0.025633  , -0.4534617 ,  0.08365186],
        [ 1.11668203, -1.45857374,  0.9146205 , -0.09127151,  0.24294641],
        [-1.24609861, -0.43112455,  9.67206337,  0.12585216,  0.99297232],
    ])
    if np.allclose(targets, expected_targets):
        ...
        print('Your targets:')
        print(targets)
        print("\nYour compute_q_targets isn't giving the correct result.")
        return
    ...
    numGames = numTraining + numEvalGames
    record = False

    games = runGames(
        test_layout, horizon, pacman, ghosts, display, numGames, record,
    ...
    winThresh = 0.6
    winThreshEC = 0.8
    if winRate < winThresh:
        print("Your win rate ({:.0%}) must be at least {:.0%} to receive full points for this question".format(winRate, winThresh))
    elif winRate < winThreshEC:
        print("Your win rate is: {:.0%}".format(winRate))
        tracker.add_points(4)
    else:
        print("Your win rate is: {:.0%}".format(winRate))
...
class DummyDeepQModel(object):
    def run(self, states):
        return nn.Constant(np.array(
            [[-1.04959974,  0.28053207, -2.4450277 ,  0.74411058, -1.92703264],
            [ 1.6450382 ,  0.49425018,  0.32449569,  0.67251539, -0.15441748],
            [ 1.06687712,  0.22145148,  1.58102358,  0.12312108, -1.4874284 ],
            [ 0.97399501,  0.77556107, -1.36586018, -0.31102273, -0.49515438],
            [ 1.43362316, -0.1287049 ,  0.49881204,  0.51658309, -0.41563865],
            [ 0.21826511,  0.50403613, -1.19492753, -0.18776298, -0.88418301],
            [-0.47202491,  0.14364718, -0.06350131, -0.18619758, -0.62891215],
            [-1.97911112, -2.16748451, -0.025633  , -0.4534617 ,  0.08365186],
            [ 1.11668203, -1.45857374,  1.3463116 , -0.09127151,  0.24294641],
            [-1.24609861, -0.43112455, -1.23251966,  0.12585216,  0.99297232]]))

class DummyTargetModel(object):
    """
    A model that always returns a fixed result
    """
    def run(self, states):
        return nn.Constant(np.array(
            [[ 0.08819462,  2.03165769,  0.92189191, -1.03580169, -0.08928181],
            [ 0.49950983, -0.82864916, -0.94426749,  0.05789389, -1.4539526 ],
            [-2.41479443,  0.9305185 , -0.21711359,  2.14946395, -0.49552893],
            [-1.22860368, -0.60937435,  0.7794898 , -0.35562207, -0.20310409],
            [-0.10142122, -0.50678051,  0.83045735,  0.20845483, -0.24574913],
            [-0.63295229,  0.12335428,  0.53925044,  1.09243133, -0.13011682],
            [ 1.92232003, -0.52791338,  0.14433581,  0.73885423, -1.31706257],
            [ 0.3460978 ,  0.63268648, -1.17371911,  0.69449129, -2.17870273],
            [-0.66371351,  0.7160056 , -0.9479405 , -0.38460784, -0.22808826],
            [-0.05612903, -0.21559333,  0.51263018, -0.55006547,  0.74673708]]))
