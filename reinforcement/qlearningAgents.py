...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for act in possibleActions:
            targetQValue = self.getQValue(state, act)
            actionValues[act] = targetQValue
            if targetQValue > bestQValue:
                bestQValue = targetQValue
        bestActions = [arg for arg,val in actionValues.items() if val == bestQValue]
        # There can be multiple actions that are good randomly selecting one of it
    ...
    def getAction(self, state):
        ...
        """
        possibleAction = self.getLegalActions(state)
        if len(possibleAction) == 0:
            return None
        if not util.flipCoin(self.epsilon):
            action = self.getPolicy(state)
        else:
            action = random.choice(possibleAction)
        return action
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for feature in features:
            uWeight = self.alpha * diff * features[feature]
            self.weights[feature] += uWeight
